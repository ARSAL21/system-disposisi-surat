...
<template>
    <Card class="border-blue-200/80 py-0 shadow-sm dark:border-blue-950">
        <CardHeader class="border-b p-5 sm:p-6">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div class="flex items-start gap-3">
                    <span class="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-blue-500/10 text-blue-700 dark:text-blue-300">
        ...
        <CardContent class="grid gap-4 p-5 sm:p-6">
            <div class="flex items-start gap-3 rounded-2xl border border-violet-200 bg-violet-50/65 p-4 dark:border-violet-900 dark:bg-violet-950/25">
                <Send class="mt-0.5 size-5 shrink-0 text-violet-700 dark:text-violet-300" aria-hidden="true" />
                <div>
                    <p class="text-xs text-muted-foreground">Dikirim oleh</p>
                    <p class="mt-1 font-semibold">{{ disposition.sender.name }}</p>
            ...
            >
                <MessageSquareText class="mt-0.5 size-5 shrink-0 text-muted-foreground" aria-hidden="true" />
                <div>
                    <p class="text-xs text-muted-foreground">Catatan tambahan</p>
            ...
            <div class="flex items-start gap-3 rounded-2xl border border-dashed p-4 text-sm">
                <RouteIcon class="mt-0.5 size-5 shrink-0 text-amber-700 dark:text-amber-300" aria-hidden="true" />
                <div>
                    <p class="font-semibold">Tahap berikutnya: penerima jamak</p>
                    <p class="mt-1 leading-6 text-muted-foreground">
                        Pada tahap position-based routing ini, Asisten baru dapat
                        menerima dan memeriksa disposisi. Penerusan kepada satu
                        atau beberapa Kepala Bagian akan diaktifkan pada tahap
                        M6 berikutnya.
                    </p>
                </div>
            </div>
