...
<template>
    <section
        ...
        aria-label="Filter surat keluar"
    >
        <div
            class="grid gap-3 md:grid-cols-[minmax(15rem,1fr)_repeat(3,minmax(9rem,0.42fr))_auto]"
        >
            <label class="relative">
                ...
                <Input
                    ...
                    class="h-11 pl-9"
                    placeholder="Cari nomor, agenda, instansi, atau perihal..."
                    @update:model-value="
                    ...
                    @change="
                        emit('change', {
                            status: ($event.target as HTMLSelectElement)
                                .value as OutgoingLetterFilters['status'],
                        })
                    "
                >
                    <option value="">Semua status</option>
                    <option value="AUTHORIZED">Menunggu nomor</option>
                    <option value="NUMBER_ASSIGNED">
                        Nomor sudah diberikan
                    </option>
                    <option value="SEKDA_REVIEW">
                        Menunggu pengesahan Sekda
                    </option>
                    <option value="AWAITING_MANUAL_SIGNATURE">
                        Menunggu tanda tangan Sekda
                    </option>
                    <option value="MANUAL_SCAN_REVIEW">
                        Menunggu pemeriksaan scan
                    </option>
                    <option value="READY_FOR_DELIVERY">Siap dikirim</option>
                    <option value="SIGNED_DOCUMENT_UPLOADED">
                        Menunggu verifikasi
                    </option>
            ...
            <label>
                ...
                <select
                    ...
                    @change="
                        emit('change', {
                            origin: ($event.target as HTMLSelectElement)
                                .value as OutgoingLetterFilters['origin'],
                        })
                    "
                >
                    <option value="">Semua sumber</option>
                    <option value="RESPONSE">Balasan surat masuk</option>
                    <option value="STANDALONE">Surat keluar mandiri</option>
                </select>
            </label>
            <label>
