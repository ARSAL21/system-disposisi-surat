...
<template>
    <section
        ...
        aria-labelledby="outgoing-register-heading"
    >
        <header
            class="flex flex-wrap items-center justify-between gap-3 border-b px-5 py-4 sm:px-6"
        >
            <div>
                <p
                ...
                <h2
                    id="outgoing-register-heading"
                    class="mt-1 text-lg font-semibold tracking-tight"
                >
                    Surat yang perlu ditindaklanjuti
                </h2>
            </div>
            <Badge variant="outline" class="rounded-full"
                >{{ letters.length }} ditampilkan</Badge
            >
        </header>
        ...
        <div v-if="letters.length" class="divide-y">
            <article
                ...
                class="group grid gap-4 p-5 transition-colors hover:bg-muted/25 sm:p-6 xl:grid-cols-[minmax(0,1fr)_17rem_12rem] xl:items-center"
            >
                <div class="min-w-0">
                    ...
                    <h3
                        class="mt-3 text-base leading-snug font-semibold sm:text-lg"
                    >
                        {{ letter.subject }}
                    </h3>
                    <div
                    ...
                    >
                        <p class="flex min-w-0 items-center gap-2">
                            <Building2 class="size-3.5 shrink-0" /><span
                                class="truncate"
                                >{{
                                    letter.origin === 'RESPONSE'
                                        ? letter.sender_organization_name
                                        : letter.originating_unit_name
                                }}</span
                            >
                        </p>
                        <p class="flex items-center gap-2">
                            <Inbox class="size-3.5 shrink-0" />{{
                                letter.origin === 'RESPONSE'
                                    ? `Agenda masuk ${letter.incoming_agenda_number}`
                                    : 'Surat mandiri unit teknis'
                            }}
                        </p>
                        <p class="flex items-center gap-2">
                            <UserRoundCheck class="size-3.5 shrink-0" /><span
                                class="truncate"
                                >{{ letter.signatory_position }} ·
                                {{
                                    letter.signatory_name ?? 'Pejabat aktif'
                                }}</span
                            >
                        </p>
                        <p class="flex items-center gap-2">
                ...
                <div class="rounded-2xl border bg-background p-4">
                    ...
                    >
                        <template v-for="index in 5" :key="index">
                            <span
                                ...
                                :class="
                                    letter.status === 'WITHDRAWN'
                                        ...
                                        : index - 1 <= stageIndex(letter.status)
                            ...
                            <span
                                ...
                                :class="
                                    letter.status !== 'WITHDRAWN' &&
                                    index <= stageIndex(letter.status)
                                        ? 'bg-indigo-600'
                                        : 'bg-border'
                                "
                            />
                    ...
                    <p class="mt-3 truncate font-mono text-xs font-semibold">
                        {{ letter.outgoing_number ?? 'Nomor belum diberikan' }}
                    </p>
                </div>

                <div
                    class="flex items-center justify-between gap-3 xl:flex-col xl:items-stretch"
                >
                    <p
                        class="text-xs font-medium text-muted-foreground xl:text-center"
                    >
                        {{
                            letter.next_action_label ??
                            (letter.status === 'WITHDRAWN'
                                ? 'Mandat dihentikan'
                                : 'Proses selesai')
                        }}
                    ...
                    <Button as-child class="min-h-11 rounded-xl">
                        <Link :href="letter.links.detail"
                            >Buka detail <ArrowRight class="size-4"
                        /></Link>
                    </Button>
                </div>
            </article>
        ...
        <div v-else class="grid min-h-72 place-items-center p-8 text-center">
            <div class="max-w-sm">
                <span
                    class="mx-auto grid size-12 place-items-center rounded-2xl bg-muted text-muted-foreground"
                    ><Inbox class="size-5"
                /></span>
                ...
                <p class="mt-2 text-sm leading-6 text-muted-foreground">
                    Ubah pencarian atau tampilkan kembali seluruh status
                    penerbitan.
                </p>
                <Button
                    type="button"
        ...
        <footer
            ...
            class="border-t bg-muted/15 px-5 py-4 sm:px-6"
        >
