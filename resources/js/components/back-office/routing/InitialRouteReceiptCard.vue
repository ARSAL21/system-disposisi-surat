...
<template>
    ...
    >
        ...
        <div class="mt-5 space-y-3.5">
            <!-- Target Pimpinan -->
            <div
                class="flex items-start gap-3 rounded-2xl border border-indigo-500/25 bg-indigo-500/5 p-4 dark:bg-indigo-950/30"
            >
                <Crown
                ...
                <div class="min-w-0">
                    <span
                        class="font-mono text-[10px] font-bold text-indigo-700 uppercase dark:text-indigo-300"
                    >
                        Pimpinan Penerima
                    </span>
                    <p
            ...
            <!-- Routed By -->
            <div
                class="flex items-start gap-3 rounded-2xl border border-border/70 bg-muted/40 p-4"
            >
                <UserRoundCheck
                ...
                <div class="min-w-0 text-xs">
                    ...
                    <p class="font-bold text-foreground">
                        {{ route.routed_by.name }}
                    </p>
                    <p class="text-muted-foreground">
                        {{ route.routed_by.position }}
                        <template v-if="route.routed_by.unit"
                            >· {{ route.routed_by.unit }}</template
                        >
                    </p>
                    <p
                        class="mt-1 font-mono text-[10px] text-muted-foreground tabular-nums"
                    >
            ...
            <div
                class="flex items-center gap-2 rounded-2xl bg-muted/30 p-3 text-[11px] text-muted-foreground"
            >
                <ShieldCheck class="size-4 shrink-0 text-indigo-500" />
                <span
                    >Identitas pencatat dan jabatan historis divalidasi oleh
                    server.</span
                >
            </div>
        </div>
