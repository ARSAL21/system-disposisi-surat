...
<template>
    ...
    >
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 pb-5 dark:border-border/40">
            <Link
                :href="backHref"
        ...
        <div class="mt-5 flex flex-col justify-between gap-6 lg:flex-row lg:items-start">
            <div class="space-y-3 max-w-3xl">
                <div class="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">
                    <FileText class="size-4" />
                    <span>Naskah Dinas Masuk · Lembar Disposisi Eksekutif</span>
                </div>

                <h1 class="font-['Syne',sans-serif] text-2xl font-extrabold tracking-tight text-foreground sm:text-3xl lg:text-4xl leading-snug">
                    {{ letter.subject }}
                </h1>

                <div class="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground pt-1">
                    <div class="flex items-center gap-1.5">
                        <Building2 class="size-4 text-indigo-500" />
                        <span>Instansi: <strong class="text-foreground">{{ letter.sender_organization_name }}</strong></span>
                    </div>

                    <div class="flex items-center gap-1.5 tabular-nums">
                        <CalendarClock class="size-4 text-muted-foreground" />
                        <span>Diterima: <strong class="text-foreground">{{ formatRoutingDateTime(letter.received_at) }}</strong></span>
                    </div>
                </div>
            </div>
            ...
            >
                <div class="flex size-10 items-center justify-center rounded-xl bg-indigo-600 text-white shadow-xs">
                    <Crown class="size-5" />
                </div>
                <div>
                    <span class="font-mono text-[10px] font-bold uppercase tracking-wider text-indigo-700 dark:text-indigo-300">
                    ...
                    <p class="text-[11px] text-muted-foreground">
                        Pejabat: {{ letter.current_route.target_position.holder_name }}
                    </p>
                </div>
            </div>
        </div>
