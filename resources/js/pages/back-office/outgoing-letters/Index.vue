...
import SekdaApprovalQueue from '@/components/back-office/outgoing-letters/SekdaApprovalQueue.vue';
import StandaloneNumberingQueue from '@/components/back-office/outgoing-letters/StandaloneNumberingQueue.vue';
import {
    previewOutgoingFilters,
    previewOutgoingLetters,
    previewOutgoingSummary,
    previewSekdaApprovalQueue,
    previewStandaloneNumberingQueue,
} from '@/lib/outgoingLetterPreview';
import type {
    OutgoingLetterFilters,
    OutgoingLetterPagination,
    OutgoingRegisterPageProps,
} from '@/types';
...
const filters = reactive<OutgoingLetterFilters>({
    search:
        ...
        (previewMode.value ? previewOutgoingFilters.search : ''),
    status:
        props.filters?.status ??
        (previewMode.value ? previewOutgoingFilters.status : ''),
    source:
        props.filters?.source ??
        (previewMode.value ? previewOutgoingFilters.source : ''),
    origin:
        props.filters?.origin ??
        (previewMode.value ? previewOutgoingFilters.origin : ''),
    year:
        props.filters?.year ??
        (previewMode.value ? previewOutgoingFilters.year : ''),
});

const previewLetters = computed(() => {
    const search = filters.search.trim().toLocaleLowerCase('id-ID');

    return previewOutgoingLetters.filter(
        (letter) =>
            (!search ||
                [
                    letter.subject,
                    letter.incoming_agenda_number ?? '',
            ...
            (!filters.origin || letter.origin === filters.origin) &&
            (!filters.year ||
                String(new Date(letter.authorized_at).getFullYear()) ===
                    filters.year),
    );
});
...
const pagination = computed<OutgoingLetterPagination>(() =>
    previewMode.value
        ? {
              current_page: 1,
              last_page: 1,
              from: letters.value.length ? 1 : 0,
...
function resetFilters(): void {
    Object.assign(filters, {
        ...
        origin: '',
        year: '',
    } satisfies OutgoingLetterFilters);
    void refresh();
}
</script>
...
<template>
    ...
    <main class="flex flex-1 flex-col gap-5 bg-muted/15 p-4 sm:p-6 lg:p-8">
        <div class="mx-auto flex w-full max-w-[100rem] flex-col gap-5">
            ...
            <StandaloneNumberingQueue
                :items="
                    previewMode
                        ? previewStandaloneNumberingQueue
                        : (props.numbering_queue ?? [])
                "
                :preview="previewMode"
            />
            <SekdaApprovalQueue
                :items="
                    previewMode
                        ? previewSekdaApprovalQueue
                        : (props.sekda_approval_queue ?? [])
                "
            />
            <OutgoingRegisterFilters
                :filters="filters"
            ...
            <OutgoingRegisterList :letters="letters" @reset="resetFilters">
                <template #pagination
                    ><OutgoingRegisterPagination :pagination="pagination"
                /></template>
