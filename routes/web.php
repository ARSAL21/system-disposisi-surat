...
use App\Http\Controllers\BackOffice\UserManagement\UserStatusController;
...
Route::get('account-invitations/{publicId}', [AccountInvitationController::class, 'show'])
            ...
            Route::middleware('critical-mfa')->group(function (): void {
                ...
                if (app()->environment('local', 'testing')) {
                    ...
                    Route::inertia('previews/users', 'back-office/users/Index', ['preview' => true])
                        ->name('previews.users.index');
                    Route::inertia('previews/users/{id}', 'back-office/users/Show', ['preview' => true])
                        ->name('previews.users.show');
                }
                ...
                Route::middleware('can:'.PermissionName::ViewUsers->value)
                    ->group(function (): void {
                        Route::get('users', [UserController::class, 'index'])
                            ->name('users.index');
                        Route::get('users/{managedUser}', [UserController::class, 'show'])
                            ->name('users.show');
                    });

                Route::middleware('can:'.PermissionName::InviteUsers->value)
                    ->group(function (): void {
                        Route::post('users/invitations', [UserInvitationController::class, 'store'])
                            ->middleware('throttle:user-invitation-create')
                            ->name('users.invitations.store');
                        Route::post('users/invitations/{userInvitation}/resend', [UserInvitationController::class, 'resend'])
                            ->name('users.invitations.resend');
                        Route::post('users/invitations/{userInvitation}/revoke', [UserInvitationController::class, 'revoke'])
                            ->name('users.invitations.revoke');
                    });

                Route::middleware('can:'.PermissionName::ManageUserStatus->value)
                    ->group(function (): void {
                        Route::patch('users/{managedUser}/status', [UserStatusController::class, 'update'])
                            ->middleware('throttle:user-status-mutation')
                            ->name('users.status.update');
                    });

                Route::middleware('can:'.PermissionName::ManageUserSecurity->value)
                    ->group(function (): void {
                        Route::post('users/{managedUser}/sessions/revoke', [UserSecurityController::class, 'revokeSessions'])
                            ->middleware('throttle:user-security-mutation')
                            ->name('users.security.sessions.revoke');
                        Route::post('users/{managedUser}/password-reset', [UserSecurityController::class, 'sendPasswordReset'])
                            ->middleware('throttle:user-security-mutation')
                            ->name('users.security.password-reset');
                        Route::post('users/{managedUser}/two-factor/reset', [UserSecurityController::class, 'resetTwoFactor'])
                            ->middleware([
                                'mfa',
                                RequirePassword::using(
                                    'back-office.password.confirm',
                                ...
                                'throttle:user-security-mutation',
                            ])
                            ->name('users.security.two-factor.reset');
                    });
            });
        });
});
